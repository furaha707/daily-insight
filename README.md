...
이 문구는 고정이며 사용자가 바꿀 수 없습니다 — 폼에는 읽기 전용 미리보기만 표시되고, `{title}`/`{url}` 대신 "아티클 제목"/"아티클 url"이라는 한글 라벨로 보여줍니다. 서버(`PUT /api/notifications`)도 클라이언트가 보낸 커스텀 템플릿 값을 무시하고 항상 이 고정 문구를 저장하므로, API를 직접 호출해도 우회할 수 없습니다.
...
- **최종 발송 미리보기**: 고정 템플릿 + 위 추가 링크 + "데일리인사이트 바로가기" 링크(teamId 포함, Webhook URL을 입력하면 실시간으로 채워짐)까지 합쳐서, 실제로 Slack에 어떤 메시지가 갈지 그대로 보여줍니다.
